...
@Component({
  selector: 'app-vdk-item',
  templateUrl: './vdk-item.component.html',
  styleUrls: ['./vdk-item.component.scss']
})

export class VdkItemComponent implements OnInit, OnDestroy {
  ...
  //       this.service.getRDCZ(this.doc.carkod).subscribe((resp: SolrResponse) => {
  //         this.rdczDocs = resp.response.docs;
  //       });
  //     }
  //   }
  // }
  ...
  openLink(id?: string) {
    if (id) {
      window.open(this.config.vdkAPI + '/original?id=' + id);
    } else {
      window.open(this.config.vdkAPI + '/original?id=' + this.doc.id[0]);
    }
  }
