...
import cz.incad.utils.Interval;
import java.io.File;
...
public class HarvesterJobData {
  ...
  private final AppJobData jdata;

  public HarvesterJobData(AppJobData jdata) throws Exception {
    try {
      
      this.jdata = jdata;
      this.configFile = jdata.getConfigFile();

      this.homeDir = InitServlet.CONFIG_DIR;

      //String path = this.homeDir + this.configFile + ".json";
      File fdef = FileUtils.toFile(HarvesterJobData.class.getResource("oai.json"));
...
//        }
      ...
      Logger.getLogger(HarvesterJobData.class.getName()).log(Level.SEVERE, null, ex);
  ...
  public AppJobData getAppJobData() {
    return jdata;
  }
}
